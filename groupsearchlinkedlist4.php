...
$hashgroupname = $_REQUEST["groupname"];
...
$select_groupname = $conn->prepare("SELECT `shorthashgroupname` FROM `grouplist` WHERE `hashgroupname` = ?");
$select_groupname->bind_param("s", $hashgroupname);
$select_groupname->execute();
$result_groupname = $select_groupname->get_result();
if ($result_groupname->num_rows != 1) {
    echo $hashgroupname;
...
$select = "SELECT `id`, `specificquestion` FROM `$groupname2`";
if ($result = $conn->query("$select")) { 
$combinedstring = "";
if (mysqli_num_rows($result) > 0) {
while ($row = $result->fetch_assoc()) {
      similar_text ($row['specificquestion'], $searchname, $percent);
      if ($percent >= 50) {
        $id = $row['id'];
...
if ($combinedstring == "") {
    echo "";
     ...
     foreach ($searchresult as $friendresult) {
         ...
         $rid = $exploding[1];
        $select2 = $conn->prepare("SELECT `specificquestion`, `questionuuid`, `points` FROM `$groupname2` WHERE `id` = ?");
        ...
        echo '<a class = "question_link" href = "displayanswer.php?questionid='.$row2['questionuuid'].'&groupname='.$hashgroupname.'">';
         echo stripslashes($row2['specificquestion']);
         echo '</a>';
         echo '</div>';
         echo '</div>';
