...
$selectpoint1 = $conn->prepare("SELECT askeduser, points, userpointed FROM `shareideaquestions` WHERE uuid = ?");
...
$update_points1 = $conn->prepare("UPDATE `shareideaquestions` SET points = ?, userpointed = ? WHERE uuid = ?");
...
if ($update_points1->execute()) { 
 ...
 mysqli_select_db($conn, $created_db);
    ...
    $selectpoint2 = $conn->prepare("SELECT points FROM `siquestions` WHERE uuid = ?");
    ...
    $update_points2 = $conn->prepare("UPDATE `siquestions` SET points = ? WHERE uuid = ?");
    ...
    if ($update_points2->execute()) {
        ...
        if ($update_points3->execute()) {
            $selectpoint4 = $conn->prepare("SELECT sipoints FROM `userslist` WHERE hashusername = ?");
            ...
            $points4 = (int)$rowpoint4['sipoints'];
            mysqli_free_result($resultpoint4);
            
            $updatedpoints4 = $points4 + 1;
            
            $update_points4 = $conn->prepare("UPDATE `userslist` SET sipoints = ? WHERE hashusername = ?");
