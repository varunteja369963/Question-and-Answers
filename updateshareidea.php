...
if ($questionid == "") {
  ...
  die();
   ...
   $question = mysqli_real_escape_string($conn, $question2);
    ...
    $selectgroupname = $conn->prepare("SELECT `askeduser` FROM `shareideaquestions` WHERE `uuid` = ?");
...
$update_question = $conn->prepare("UPDATE `shareideaquestions` SET `tags` = ?, `precisequestion` = ?, `question` = ? WHERE `uuid` = ?");
