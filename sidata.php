...
$select_quote = $conn->prepare("SELECT `hashusername`, `gmail`, `quote`, `points`, `sipoints`, `profileaddr` FROM `userslist` WHERE `username` = ?");
...
header('location: loginform.php');
    ...
    $points_in_smp = $row_quote['sipoints'];
...
echo '<div class = "points_in_this_group" title = "points earned in shareidea" style = "display: flex;">';
...
echo '</div>';
$select2 = "SELECT `precisequestion`, `uuid`, `points` FROM `siquestions` ORDER BY id DESC";
...
while ($row2 = $result2->fetch_assoc()) {
    ...
    echo '<a id = "question_link" href = "shareidearesult.php?uuid='.$row2['uuid'].'">';
...
echo '</div>';
$select3 = "SELECT `precisequestion`, `uuid` FROM `sianswers` ORDER BY id DESC";
if ($result3 = $conn->query($select3)) { 
echo '<div class = "answers_list">';
while ($row3 = $result3->fetch_assoc()) {
    echo '<div class = "answer_info">';
    echo '<div class = "asked_answers">';
    echo '<a id = "answer_link" href = "shareidearesult.php?uuid='.$row3['uuid'].'">';
