...
session_start();
  include_once('connection5.php');
    ...
    $selectgroupname = $conn->prepare("SELECT `precisequestion` FROM `shareideaquestions` WHERE `uuid` = ?");
...
$select_repeated = $conn->prepare("SELECT `uuid` FROM `sianswers` WHERE `uuid` = ?");
...
if($result_repeated->num_rows > 0) {
  echo '1';
    ...
    $insertstoringanswers = $conn->prepare("INSERT INTO `sianswers` (precisequestion, uuid) 
    VALUES (?, ?)");
