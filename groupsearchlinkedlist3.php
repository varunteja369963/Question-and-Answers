...
$select = "SELECT `id`, `precisequestion` FROM `solvemyproblemquestions`";
...
if ($combinedstring == "") {
    echo "";
     ...
     foreach ($searchresult as $friendresult) {
         ...
         $rid = $exploding[1];
        $select2 = $conn->prepare("SELECT `precisequestion`, `uuid`, `points` FROM `solvemyproblemquestions` WHERE `id` = ?");
        ...
        echo '<a class = "question_link" href = "displayresult.php?uuid='.$row2['uuid'].'">';
